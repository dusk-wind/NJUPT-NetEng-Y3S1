% =========================================================================
% 课设任务拓展: 使用梯度提升决策树 (GBDT) 进行语义分割 (修正版)
% =========================================================================
clear; clc; close all;
...
model_filename = 'gbdt_model_99_images.mat'; % GBDT模型的新文件名
...
if exist(model_filename, 'file')
    fprintf('发现已训练好的GBDT模型文件: %s\n正在加载模型...\n', model_filename);
    load(model_filename, 'gbdtModel');
    fprintf('模型加载成功！\n');
else
    fprintf('未发现已训练好的模型，开始新的GBDT训练流程...\n');
    ...
    fprintf('\n开始训练梯度提升决策树(GBDT)模型...\n');
    tic;
    % --- 核心修正：使用支持多分类的 'AdaBoostM2' 提升方法 ---
    gbdtModel = fitcensemble(allTrainFeatures, allTrainLabels, ...
        'Method', 'AdaBoostM2', ...
    ...
    trainingTime = toc;
    fprintf('GBDT模型训练完成！耗时: %.2f 秒 (约 %.1f 分钟)\n', trainingTime, trainingTime/60);

    % 2d. 保存新模型
    fprintf('正在将训练好的模型保存到文件: %s\n', model_filename);
    save(model_filename, 'gbdtModel'); % 注意变量名是 gbdtModel
    fprintf('模型保存成功！\n');
end
...
tic;
predictedLabels = predict(gbdtModel, testFeatures); % 使用 gbdtModel 进行预测
predictionTime = toc;
...
figure('Name', 'GBDT Segmentation Results', 'Position', [50, 50, 1600, 450]);
subplot(1, 4, 1);
